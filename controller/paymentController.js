...
exports.createPayment = async (req, res) => {
  try {
    ...
    const payment = new Payment({ booking, amount, method, transactionRef });
    await payment.save();
...
exports.updatePayment = async (req, res) => {
  try {
    ...
    const payment = await Payment.findByIdAndUpdate(req.params.id, { status }, { new: true }).populate("booking");
    if (!payment) return res.status(404).json({ message: "Payment not found" });
